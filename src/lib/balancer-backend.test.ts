...
import { kjorBalansert, poolFor, poolStatus, resetBalancer, rutingsRekkefolge } from "../../agent/lib/balancer.mjs";

const node = (id, extra = {}) => ({ id, navn: id, baseUrl: `http://${id}:11434/v1`, oppgaver: ["chat"], aktiv: true, vekt: 1, ...extra });
...
describe("lastbalansering", () => {
  ...
  it("fordeler videre til neste node når en node feiler", async () => {
    ...
    const r = await kjorBalansert(liste, async (n) => {
      brukt.push(n.id);
      if (n.id === brukt[0] && brukt.length === 1) throw new Error("nede");
      return "ok";
    });
  ...
  it("rapporterer poolstatus med karantene", async () => {
    const liste = [node("a")];
    await expect(kjorBalansert(liste, async () => { throw new Error("nede"); })).rejects.toThrow();
